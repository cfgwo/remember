...
/**
 * encapsulation of the ContentResolver for a single URI <p> Uses ContentResolver instead of ContentProviderClient or other mechanism to simplify code and to make this object thread safe. Future revisions to the code could have ContentProviderClient (which takes 50% of the time during access, once setup, than ContentResolver( did independent testing to find this out.))
